...
Next, we apply variance decomposition (Equation~\eqref{fairness_fairXplainer_eq:variance_decomposition_set_notation}) to estimate the influence of each $ \nonsensitive_{\mathbf{S}} $. From Equation~\eqref{fairness_fairXplainer_eq:variance_decomposition_set_notation} in Chapter~\ref{fairness_fairXplainer_sec:preliminaries}, we obtain that the variance of $ B_{\mathbf{a}} $ can be decomposed as 
...
\subsection{Accuracy: Equalized Odds \& Predictive Parity via FIFs.}
...
\subsection{Execution Time: Equalized Odds \& Predictive Parity via FIFs}
